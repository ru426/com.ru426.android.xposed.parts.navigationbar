...
public class OptionalKeyReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (XUtil.OPTIONAL_BUTTON_CLICKED.equals(intent.getAction())) {
		...
		if (modeStr.equals(OptionalKeyAppLauncherActivity.MODE_PREVIOUS)) {
			...
			List<ActivityManager.RecentTaskInfo> recentTaskList = am.getRecentTasks(8, ActivityManager.RECENT_WITH_EXCLUDED);
			PackageManager pm = context.getPackageManager();
			Intent queryIntent = new Intent();
			queryIntent.setAction(Intent.ACTION_MAIN);
			queryIntent.addCategory(Intent.CATEGORY_HOME);
			final List<ResolveInfo> homeAppList = pm.queryIntentActivities(queryIntent, 0);
